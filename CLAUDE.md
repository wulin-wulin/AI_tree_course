...
本文件是项目级长期记忆入口，供后续参与本仓库的智能体优先阅读。它只保留必须立即知道的项目信息和工作边界；更详细的开发规范放在 `.claude/` 中。
...
- `.claude/README.md`：说明 `.claude/` 目录用途。
- `.claude/development.md`：详细开发规范、模块说明、验证和产品注意事项。
- `.claude/git.md`：Git 分支、提交信息和提交前检查规范。

## 首次运行项目需要安装的 Claude Code 插件

项目通过 `.claude/settings.json` 声明启用两个项目级插件。首次克隆仓库或第一次运行时，需要本地安装这两个插件包（Claude Code 会从 `claude-plugins-official` marketplace 自动拉取）：
...
安装完成后通过 `claude plugin list` 确认两个插件都是 `Scope: project` 且 `enabled`。插件包缓存在 `~/.claude/plugins/cache/`，不会进入仓库。
